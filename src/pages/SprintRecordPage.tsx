      ...
      {showShareSheet && (
        <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.4)', zIndex: 100 }} onClick={() => setShowShareSheet(false)}>
          <div style={{ position: 'fixed', bottom: 0, left: 0, right: 0, maxWidth: '430px', margin: '0 auto', backgroundColor: '#FFFFFF', borderRadius: '24px 24px 0 0', padding: '20px 20px 40px' }} onClick={(e) => e.stopPropagation()}>
            ...
            <div onClick={() => { navigator.clipboard.writeText(window.location.href); setToast('Link copied! Paste on Indeed profile.'); window.open('https://www.indeed.com', '_blank') }} style={{ display: 'flex', alignItems: 'center', gap: '14px', padding: '14px 0', borderBottom: '1px solid #F5F5F5', cursor: 'pointer' }}>
              <div style={{ width: '42px', height: '42px', borderRadius: '50%', backgroundColor: '#2164F4', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'var(--font-body)', fontSize: '16px', fontWeight: 800, color: '#FFFFFF', flexShrink: 0 }}>in</div>
              <div><p style={{ fontFamily: 'var(--font-body)', fontSize: '14px', fontWeight: 600, color: '#1A3028', margin: 0 }}>Indeed</p><p style={{ fontFamily: 'var(--font-body)', fontSize: '11px', fontStyle: 'italic', color: '#9BBFB2', margin: 0 }}>Share as proof of work to recruiters</p></div>
            </div>
